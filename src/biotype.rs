...
impl TryFrom<u8> for BioType {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        if value > 101 {
            return Err(Error::Parse(format!("invalid biotype byte: {value}")));
        }
        // Safety: all values 0..=101 correspond to valid enum variants
        Ok(unsafe { std::mem::transmute::<u8, BioType>(value) })
    }
}
